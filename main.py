...
# --- Global variable for the Node.js process ---
...
# Quieten down noisy loggers
logging.getLogger('werkzeug').setLevel(logging.WARNING)
logging.getLogger('httpx').setLevel(logging.WARNING)
logging.getLogger('telegram').setLevel(logging.WARNING)

# --- Start Node.js Server Function ---
def start_node_server():
    global node_server_process
    server_file = 'server.js'
    if os.path.exists(server_file):
        try:
            node_server_process = subprocess.Popen(['node', server_file])
            logger.info(f"Node.js server '{server_file}' started successfully.")
            atexit.register(lambda: node_server_process.terminate())
        except Exception as e:
            logger.error(f"An error occurred while starting the Node.js server: {e}")
    else:
        logger.warning(f"'{server_file}' not found. The Node.js server will not be started.")
...
# MODIFIED: handle_image function to correctly process classification results
def handle_image(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        update.message.reply_text("I had trouble downloading your image due to a network issue. Please try sending it again.")
        return
    except Exception as e:
    ...
    try:
        ...
        results = model(pil_image) # No confidence threshold needed here for classification
        
        # Check if the model produced classification probabilities
        if results[0].probs is None:
            update.message.reply_text("I'm sorry, I couldn't determine the breed from this image.")
            return

        # Get the top prediction from the probabilities
        top1_prediction = results[0].probs.top1
        top1_confidence = results[0].probs.top1conf.item() # .item() gets the number from the tensor
        
        class_id = top1_prediction
...
# MODIFIED: classify_image function to correctly process classification results
...
def classify_image():
    ...
    try:
        ...
        if results[0].probs is None:
            return jsonify({'error': 'No breed could be determined from the image.'}), 404
        ...
        return jsonify({'breedName': breed_name, 'confidence': f"{confidence:.2%}"})
    except Exception as e:
